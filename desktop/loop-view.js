 ...
 * 输入为 Loop Store、当前 Context 与独立 Console Store；输出为可创建 Loop 的启动页、自主压缩授权/状态，或承载真实 Context 图、完整会话和事实表的运行/历史壳层。
 * 具体工作流为筛选尚未绑定 Loop 的直接用户 Run，显式授予或收窄可恢复压缩权力，按生命周期呈现控制与终止态出口，并把拓扑、会话、事实和介入委托给 Console View。
...
(function (root, factory) {
  ...
  function compressionStatus(related) {
    ...
    const reduction = resolution?.actual_reduction ?? candidate?.estimated_reduction;
  ...
  function render(state, context = {}, consoleState = null) {
    ...
    return `<section class="loop-dashboard console-shell" data-loop-id="${escape(loop.loop_id)}" data-loop-status="${escape(loop.status)}">${commandBar(loop, consoleState)}${loop.waiting_reason ? `<p class="loop-waiting" data-loop-waiting role="status">${escape(loop.waiting_reason)}</p>` : ""}${compressionStatus({ ...related, snapshot: loop })}${terminal ? `<div class="loop-terminal-notice" role="status"><strong>该 Loop 已${loop.status === "completed" ? "完成" : loop.status === "failed" ? "失败" : "停止"}</strong><span>历史 Context、完整会话和事实证据仍可查看；退出不会删除审计记录。</span></div>` : ""}${consoleHtml}<details class="loop-advanced"><summary>授权、预算与目标控制</summary>${budgetDetails}${grantControls(loop)}${override}<p class="muted tiny">当前轮次 ${escape(loop.current_round_id || "—")} · 活动 Run ${escape(activeRuns.length)} · Goal R${escape(loop.goal_revision)} · Authority R${escape(loop.authority_revision)}</p></details></section>`;
  }
