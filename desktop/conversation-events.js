/*
 * 本文件把 reasoning、pending tool call 与 ToolMessage 归一为紧凑会话事件。
...
(function (root, factory) {
    ...
    for (const message of list) {
      if (["ai", "assistant"].includes(message?.role)) {
        if (message.reasoning_content) {
          records.push({ type: "reasoning", content: message.reasoning_content, messageId: message.id || "" });
        }
        if (text(message.content).trim() || message.files?.length) records.push({ type: "message", message });
        for (const call of (Array.isArray(message.tool_calls) ? message.tool_calls : [])) {
          if (!resolved.has(call.id)) records.push({ type: "tool", call, result: null, status: "pending" });
        }
  ...
  function renderReasoning(event) {
    ...
    return `<details class="conversation-event is-reasoning">
      <summary><span class="conversation-event-mark" aria-hidden="true">◇</span><strong>Think</strong><span class="conversation-event-preview">${escapeHtml(preview(full))}</span></summary>
  ...
  function renderTool(event) {
    ...
    return `<details class="conversation-event is-tool is-${escapeHtml(status)}">
      <summary><span class="conversation-event-mark" aria-hidden="true">${status === "error" ? "!" : "↳"}</span><strong>${escapeHtml(call.name || event.result?.name || "tool")}</strong><span class="conversation-event-preview">${escapeHtml(argumentSummary(args) || error)}</span><span class="conversation-event-status">${escapeHtml(statusLabel)}</span>${error ? `<span class="conversation-event-error">${escapeHtml(error)}</span>` : ""}</summary>
