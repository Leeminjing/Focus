/*
 * 本文件验证图片材料选择与独立必看控件。输入为任务材料、binding/required 状态和可控
 * image-validity API；输出为三个合法 UI 状态、选择时的新鲜度校验，以及缩略图/消息/压缩/放大
 * 四个入口不泄漏 session 的断言。具体工作流在 VM 中直接调用 app.js 的纯渲染和选择函数。
...
(async () => {
  const result = await harness.vm.runInContext(`(async () => {
    ...
    state.materialSelections.set("task", { bindings: [{ materialId: "m1", note: "" }], requiredImageIds: [], notes: {} });
    const attached = renderImageMaterial(material);
    state.materialSelections.set("task", { bindings: [{ materialId: "m1", note: "" }], requiredImageIds: ["m1"], notes: {} });
