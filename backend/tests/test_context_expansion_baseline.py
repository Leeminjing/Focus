...
输入为单 Context observation、简单 continue action 与只携带 identity 的语义派生 action；输出为 Bootstrap Curator scope、continue 与
identity 级 spawn 合同断言。具体工作流为只构造不可变合同并调用现有公开 interface，
...
def test_semantic_spawn_selects_a_frozen_opportunity_by_identity() -> None:
    ...
    with pytest.raises(ValidationError):
        PATROL_MODEL_ACTION_ADAPTER.validate_python(
            {"action": "spawn_context", "opportunity_id": "a" * 64, "work_order": "Restate the frozen work order."}
        )
