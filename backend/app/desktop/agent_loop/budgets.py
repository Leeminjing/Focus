...
输入为 grant hard/soft budgets、持久 usage、运行装备和本轮目标/失败/workspace/action/result 摘要；输出为
...
class LoopBudgetGuard:
    ...
    def evaluate(self, usage: dict[str, int], budgets: dict[str, int], next_action: str) -> BudgetDecision:
        exhausted = [name for name, limit in self.HARD_FIELDS.items() if int(usage.get(name, 0)) >= int(budgets.get(limit, 2**63 - 1))]
        if exhausted:
