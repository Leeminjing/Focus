...
from backend.app.desktop.agent_loop.compression_authority.recovery_evidence import CompressionRecoveryEvidenceReader
...
class CompressionResolutionCoordinator:
    def __init__(self, sessions: async_sessionmaker[AsyncSession], desktop_service, *, lease_seconds: int = 90) -> None:
        ...
        self._evidence = CompressionRecoveryEvidenceReader(getattr(desktop_service, "checkpointer", None))
    ...
    async def settle_run(self, session: AsyncSession, event, run, loop) -> str | None:
        ...
        evidence = await self._evidence.inspect(session, candidate, run, current) if candidate is not None else None
        if evidence is None or not evidence.matched:
            await self._mark_failed(
                ...
                action,
                round_row,
                evidence.reason if evidence is not None else "candidate_missing",
                run,
            )
            return "failed"
        await self._mark_applied(
            ...
            action=action,
            round_row=round_row,
            actual_before_tokens=evidence.actual_before_tokens,
            actual_after_tokens=evidence.actual_after_tokens,
        )
        return "applied"
            ...
            for row in rows:
                if row.status == "resuming" and row.resume_run_id:
                    ...
                    if run is not None and run.status == "success" and run.settled_at is not None:
                        current = await self._revisions.current(session, run.task_id)
                        candidate = await session.get(LoopCompressionCandidate, row.candidate_id)
                        evidence = await self._evidence.inspect(session, candidate, run, current) if candidate is not None else None
                        ...
                        if loop is not None:
                            ...
                            continue
                        row.status = "failed"
                        row.error_evidence = {"reason": "loop_missing_during_reconciliation", "run_id": run.run_id}
                        recovered += 1
                        continue
    ...
    async def _mark_applied(
        ...
        *,
        ...
        actual_before_tokens: int,
        actual_after_tokens: int,
    ) -> None:
        ...
        resolution.actual_before_tokens = actual_before_tokens
        resolution.actual_after_tokens = actual_after_tokens
    ...
    async def _mark_failed(
        ...
        resolution: LoopCompressionResolution,
        loop: AgentLoop,
        pending: LoopPendingDecision | None,
        action: LoopAction | None,
        round_row: LoopRound | None,
        reason: str,
        run: DesktopRun,
    ) -> None:
        pending = pending or await session.get(LoopPendingDecision, resolution.pending_decision_id, with_for_update=True)
        ...
        round_row = round_row or await session.get(LoopRound, resolution.round_id, with_for_update=True)
        resolution.status = "failed"
        resolution.error_evidence = {"reason": reason, "run_id": run.run_id, "checkpoint_id": run.final_checkpoint_id}
        resolution.lease_owner = None
        resolution.lease_token = None
        resolution.lease_expires_at = None
        if pending is not None:
