r"""本文件对外提供完整 Revision semantic index 的不可变合同与稳定 identity 工具。
...
再由来源与版本化 payload 计算 index identity。示例：`index = RevisionSemanticIndex.create(...)`。
"""
    ...
    def require_complete_valid_index(self) -> Self:
        ...
        for unit in self.semantic_units:
            for ref in unit.evidence_refs:
                ...
                if ref.source != self.source or ref.message_id not in known:
