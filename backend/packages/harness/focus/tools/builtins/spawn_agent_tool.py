...
输入:
    spawn_agent(task, system_prompt=None):
        ...
        runtime: ToolRuntime — LangGraph 注入；受治理安全上下文经 runtime.context 携带

输出:
    str — 子 Agent 的最终回答文本，成为主 Agent 的 ToolMessage 进入主对话

具体工作流:
    (1) 从运行上下文取回父级安全上下文；缺失即失败，不以宽松默认值继续
    (2) 从父级单调派生子级安全上下文：能力权限与工作根不超集、访问模式不更宽、权柄面不放宽
...
from focus.security.effects import DELEGATED_EXECUTION_EFFECT, declare_effect
...
def build_spawn_agent_tool() -> BaseTool:
    ...
    async def spawn_agent(task: str, runtime: ToolRuntime, system_prompt: str | None = None) -> str:
        ...
        child = await make_lead_agent(
            model_name=child_security.model_name,
            tools=select_workspace_tools(list(child_security.authorization.permissions)),
