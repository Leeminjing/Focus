...
对外提供:
    build_general_middlewares(app_config, model, context7_tools_loader, skill_names) — 按 commitment.enabled
...
输入:
    ...
    context7_tools_loader — 承诺阶段按需调用的隔离 Context7 工具加载函数
...
工作流:
    ...
    (3) 开启时要求当前模型和 Context7 加载函数，以 skill_names 构造 CommitmentMiddleware

示例:
    middlewares = build_general_middlewares(app_config, model, load_context7_tools, frozenset({"docx"}))
"""
...
def build_general_middlewares(
    ...
    if context7_tools_loader is None:
        raise RuntimeError("CommitmentMiddleware 已启用，但未提供 Context7 工具加载器")
