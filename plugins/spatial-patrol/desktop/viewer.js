/* spatial-patrol 内容查看器。输入为宿主文件工作台、同源插件 API、图片/PDF/文本文档与锚点操作，
   输出为保持内容坐标的载体视图和显式授权动作；DOCX 的 deploy/copy/continue 必须选择只读或
   修改模式，其他载体保持只读，entry.js 负责将能力注册到宿主授予的插件根。 */
(function (root) {
  "use strict";
  ...
  const ANCHOR_STATUS = Object.freeze({
    anchor: { label: "待投放", tone: "neutral" },
    deployed: { label: "执行中", tone: "active" },
    done: { label: "已完成", tone: "success" },
    dismissed: { label: "已回收", tone: "neutral" },
    needs_action: { label: "需要处理", tone: "warning" },
    invalid: { label: "锚点失效", tone: "danger" },
  });

  function presentAnchorStatus(status) {
    return ANCHOR_STATUS[status] || { label: status || "待投放", tone: "neutral" };
  }
  ...
  function refreshSelection() {
    ...
    const presented = presentAnchorStatus(anchor.status);
    panel.innerHTML = `<div class="spatial-selected-card status-${escapeHtml(anchor.status)}">
      <header><strong>${anchor.kind === "anchor" ? "空间锚点" : "空间小兵"} ${escapeHtml(anchor.spatial_id.slice(0, 8))}</strong><span class="ui-badge is-${presented.tone}">${presented.label}</span></header>
      <span class="muted tiny">第 ${anchor.page} 页 · (${anchor.x.toFixed(3)}, ${anchor.y.toFixed(3)})</span>
      ...
      <span class="spatial-selected-actions">
        ${anchor.kind === "patrol" && ["deployed", "done", "needs_action"].includes(anchor.status) ? `<button class="text-button" data-spatial-action="observe-anchor">观察</button><button class="text-button" data-spatial-action="copy-anchor">复制锚点</button>${anchor.status !== "needs_action" ? `<button class="text-button" data-spatial-action="complete-anchor">标记完成</button>` : ""}<button class="text-button danger" data-spatial-action="dismiss-anchor">回收</button>` : ""}
  ...
  function panelMarkup() {
    ...
    const typeLabel = isPdf() ? "PDF" : isDocx() ? "DOCX" : /\.md$/i.test(name) ? "MARKDOWN" : /\.txt$/i.test(name) ? "TEXT" : "IMAGE";
    return `
      ...
      <div class="file-panel-body" data-carrier="${typeLabel.toLowerCase()}">
        ${textType
      ...
      ${spatial ? `
        <footer class="file-panel-foot">
          <form id="spatialDeployForm">
            <label class="spatial-instruction-label" for="spatialInstruction">锚点指令</label><textarea id="spatialInstruction" placeholder="先点内容建立锚点，再描述任务" rows="2"></textarea>
            ${isDocx() ? `<fieldset class="spatial-permission"><legend>本次操作权限（必选）</legend><label><input type="radio" name="spatial-docx-run-mode" value="read" data-spatial-run-mode required ${state.docxRunMode === "read" ? "checked" : ""}><span><strong>只读观察</strong><small>不会修改文件</small></span></label><label><input type="radio" name="spatial-docx-run-mode" value="write" data-spatial-run-mode ${state.docxRunMode === "write" ? "checked" : ""}><span><strong>修改文档</strong><small>允许写入与删除</small></span></label></fieldset>` : ""}
            <button class="primary" type="submit">在当前锚点投放</button>
          </form>
